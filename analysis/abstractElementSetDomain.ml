...
module Make (Element : ELEMENT_DOMAIN) = struct
  ...
  let is_subsumed left set = Set.exists ~f:(fun right -> Element.less_or_equal ~left ~right) set
    ...
    if is_subsumed to_add set then
      ...
      let try_remove set existing =
        if Element.less_or_equal ~left:existing ~right:to_add then
          ...
          set
      in
      Set.add (Set.fold ~init:set set ~f:try_remove) to_add
  ...
  let widen ~iteration:_ ~previous ~next =
    join previous next |> Set.to_list |> Element.widen |> of_list


  let less_or_equal ~left ~right = Set.for_all left ~f:(fun element -> is_subsumed element right)
  ...
  type _ AbstractDomain.part +=
    | Element : Element.t AbstractDomain.part | Set : Element.t list part
  ...
  let transform (type a) (part : a part) ~(f : a -> a) set =
    ...
    | Set -> f (Set.elements set) |> of_list
    | _ ->
        Obj.extension_constructor part
        |> Obj.extension_name
