...
module Make(Config: CONFIG) (Element: AbstractDomain.S)() = struct
  ...
  module Checks =
    (val (if Config.check_invariants then
            (module WithChecks)
          else
            (module WithoutChecks)
         ) : CHECK)
  ...
  *)
  ...
  let rec is_minimal ancestors ({ element; children } as tree) =
    if is_empty_tree tree then
      Checks.false_witness ~message:(fun () -> "empty leaf.")
         ...
         && Element.less_or_equal ~left:element ~right:ancestors then
  ...
  let rec join_trees
    ...
    if must_widen_depth widen_depth then begin
      ...
      let { new_element; ancestors } =
        filter_by_ancestors ~ancestors ~element:joined_element
      in
      let children =
        join_children
          ancestors
  ...
  and join_option_trees ancestors ~widen_depth left right =
    ...
    | Some left, Some right ->
        join_trees ancestors ~widen_depth left right
    ...
    *)
    ...
    let merge_right ~key:element ~data:right_subtree accumulator =
      ...
      | None ->
          ...
          | Label.Any ->
              let join_tree =
                join_option_trees ancestors ~widen_depth None (Some right_subtree)
              in
              set_or_remove element join_tree accumulator
    in
  ...
  let collapse_tree_indices ({ element=ancestors; children } as tree) =
    let message () =
      Format.sprintf "collapse_tree_indices: %s" (show tree)
    in
  ...
  *)
  ...
  let assign_tree_path ~tree path ~subtree =
    let message () =
      Format.sprintf
        ...
        (show tree)
    in
    assign_path ~ancestors:Element.bottom ~tree path ~subtree
  ...
  let join_tree_path ~tree path ~subtree =
    let message () =
      Format.sprintf
        ...
        (show tree)
    in
    join_path ~ancestors:Element.bottom ~tree path ~subtree
    |> option_node_tree ~message
  ...
  and less_or_equal_option_tree left_option_tree right_ancestors right_option_tree =
    ...
    | Some left_tree, Some right_tree ->
        less_or_equal_tree left_tree right_ancestors right_tree
  ...
  let read ?(transform_non_leaves=fun _p element -> element) path tree =
    ...
    in
    let message () =
      Format.sprintf "read [%s] from %s" (Label.show_path path) (show tree)
    in
  ...
  let less_or_equal ~left ~right =
    less_or_equal_tree left Element.bottom right |> Checks.is_true
  ...
  let transform (type a) (part: a AbstractDomain.part) ~(f: a -> a) tree =
    ...
    | Path ->
        let transform_node ~path ~ancestors ~element =
          f (path, Element.join ancestors element)
        in
    ...
    | _ ->
        let transform_node ~path ~ancestors:_ ~element =
          path, Element.transform part ~f element
        in
        filter_map_tree_paths ~f:transform_node tree


  let partition (type a b) (part: a AbstractDomain.part) ~(f: a -> b) tree
    : (b, t) Map.Poly.t =
    ...
    | _ ->
        let partition ~path ~ancestors:_ ~element result =
          ...
          let distribute ~key ~data result =
            Map.Poly.update result key ~f:(update path data)
          in
