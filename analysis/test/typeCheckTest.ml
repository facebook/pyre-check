...
let create_ignore ignored_line codes =
  let location =
    let start = { Location.line = ignored_line; column = 80 } in
    let stop = { Location.line = ignored_line; column = 81 } in
    { Location.path = "test.py"; start; stop }
  in
  Source.Ignore.create ~ignored_line ~codes ~kind:Source.Ignore.PyreIgnore ~location


let test_ignore_lines _ =
  assert_type_errors
    ~ignore_lines:[create_ignore 1 []]
  ...
  assert_type_errors
    ...
    [];
  assert_type_errors
    ~ignore_lines:[create_ignore 1 []; create_ignore 2 []]
    "def foo() -> str: return 1.0\ndef bar() -> int: return '' "
    [];
  assert_type_errors
    ~ignore_lines:[create_ignore 2 []]
    "class A: pass\ndef foo() -> A: return 1"
    [];
  assert_type_errors
    ~ignore_lines:[create_ignore 1 []]
  ...
  assert_type_errors
    ...
    [];
  assert_type_errors
    ~ignore_lines:[create_ignore 1 [7]]
    "def foo() -> typing.List[str]: return 1"
    [];
  assert_type_errors
    ~ignore_lines:[create_ignore 1 [7]; create_ignore 2 [7]]
    "def foo() -> str: return 1.0\ndef bar() -> int: return '' "
    []
