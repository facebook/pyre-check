...
let assert_type_errors
  ...
  let source =
    let ignores =
      let create_ignores (ignored_line, codes) =
        Source.Ignore.create
          ~ignored_line
          ~codes
          ~location:Location.any
          ~kind:Source.Ignore.PyreIgnore
      in
      List.map ~f:create_ignores ignore_lines
    in
    let metadata =
...
let test_ignore_lines _ =
  assert_type_errors ~ignore_lines:[1,[]] "def foo() -> int: return 1.0" [];
  assert_type_errors ~ignore_lines:[1,[]] "def foo() -> str: return 1.0" [];
  assert_type_errors ~ignore_lines:[1,[]] "def foo() -> str: return" [];
  assert_type_errors ~ignore_lines:[1,[]] "def foo() -> typing.List[str]: return 1" [];
  assert_type_errors
    ~ignore_lines:[1,[]; 2,[]]
    "def foo() -> str: return 1.0\ndef bar() -> int: return '' "
    [];
  assert_type_errors ~ignore_lines:[2,[]] "class A: pass\ndef foo() -> A: return 1" [];
  assert_type_errors ~ignore_lines:[1,[]] "def foo() -> str: return bar()" [];
  assert_type_errors
    ~debug:false
    ~ignore_lines:[0,[]]
    {|
    ...
    |}
    [];
  assert_type_errors ~ignore_lines:[1,[7]] "def foo() -> typing.List[str]: return 1" [];
  assert_type_errors
