...
module StringLiteral = struct
  ...
  let create_mixed pieces =
    ...
    match pieces with
    | [] -> { value = ""; kind = String }
    | [{ Substring.kind = Literal; value }] -> { value; kind = String }
    | _ ->
        let value =
          pieces |> List.map ~f:(fun { Substring.value; _ } -> value) |> String.concat ~sep:""
        in
...
let rec convert_to_new ({ Node.location; value } as expression) =
  let rec convert expression access =
    ...
    | Some { Node.value = expression; location }, [Access.Identifier identifier] ->
        Name
          (Name.Attribute
    ...
    | Some { Node.value = expression; location }, [Call arguments] ->
        ...
        |> Node.create ~location
    | None, [Identifier identifier] -> Name (Name.Identifier identifier) |> Node.create ~location
    ...
    | _, Identifier identifier :: access ->
        Name
          (Name.Attribute
             { base = convert expression access; attribute = identifier; special = false })
        |> Node.create ~location
    | _, Call arguments :: access ->
        Call { callee = convert expression access; arguments = convert_arguments arguments }
        |> Node.create ~location
    | Some { Node.value = Await expression; location }, [] ->
        Await (convert_to_new expression) |> Node.create ~location
    | Some { Node.value = BooleanOperator { BooleanOperator.left; right; operator }; location }, []
      ->
        BooleanOperator
          { BooleanOperator.left = convert_to_new left; right = convert_to_new right; operator }
        |> Node.create ~location
    | Some { Node.value = Call { callee; arguments }; location }, [] ->
        Call { callee = convert_to_new callee; arguments } |> Node.create ~location
    | ( Some
        ...
        [] ) ->
        ComparisonOperator
          { ComparisonOperator.left = convert_to_new left; right = convert_to_new right; operator }
        |> Node.create ~location
    | Some { Node.value = Dictionary { Dictionary.entries; keywords }; location }, [] ->
        ...
        |> Node.create ~location
    | ( Some
          { Node.value = DictionaryComprehension { Comprehension.element; generators }; location },
        [] ) ->
        DictionaryComprehension
          { Comprehension.element = convert_entry element;
            generators = List.map ~f:convert_generator generators
    ...
    | Some { Node.value = List elements; location }, [] ->
        List (List.map ~f:convert_to_new elements) |> Node.create ~location
    ...
    | Some { Node.value = Name (Name.Attribute ({ base; _ } as name)); location }, [] ->
        Name (Name.Attribute { name with base = convert_to_new base }) |> Node.create ~location
    | Some { Node.value = Set elements; location }, [] ->
        Set (List.map ~f:convert_to_new elements) |> Node.create ~location
    ...
    | Some { Node.value = Starred (Starred.Once expression); location }, [] ->
        Starred (Starred.Once (convert_to_new expression)) |> Node.create ~location
    | Some { Node.value = Starred (Starred.Twice expression); location }, [] ->
        Starred (Starred.Twice (convert_to_new expression)) |> Node.create ~location
    | Some { Node.value = String { StringLiteral.value; kind }; location }, [] ->
        let kind =
          ...
          | _ -> kind
        in
        String { StringLiteral.value; kind } |> Node.create ~location
    ...
    | Some { Node.value = Tuple elements; location }, [] ->
        Tuple (List.map ~f:convert_to_new elements) |> Node.create ~location
    | Some { Node.value = UnaryOperator { UnaryOperator.operand; operator }; location }, [] ->
        UnaryOperator { UnaryOperator.operand = convert_to_new operand; operator }
        |> Node.create ~location
    | Some { Node.value = Yield expression; location }, [] ->
        Yield (expression >>| convert_to_new) |> Node.create ~location
    | Some expression, _ -> expression
    | _ -> Name (Name.Identifier "") |> Node.create ~location
  in
  match value with
  | Access (SimpleAccess access) -> convert None (List.rev access)
  | Access (ExpressionAccess { expression; access }) -> convert (Some expression) (List.rev access)
  | _ -> convert (Some expression) []
