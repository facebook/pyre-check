...
let rec convert statement =
  ...
  | Assert { Assert.test; message; origin } ->
      Assert
        { Assert.test = Expression.convert test; message = message >>| Expression.convert; origin }
      |> Node.create ~location
  | Class { Class.name; bases; body; decorators; docstring } ->
      ...
      |> Node.create ~location
  | Define
      { signature = { name; parameters; decorators; return_annotation; async; parent; docstring };
        body
      } ->
      ...
      |> Node.create ~location
  | Delete expression -> Delete (Expression.convert expression) |> Node.create ~location
  | Expression expression -> Expression (Expression.convert expression) |> Node.create ~location
  ...
  | If { If.test; body; orelse } ->
      ...
      |> Node.create ~location
  | Raise expression -> Raise (expression >>| Expression.convert) |> Node.create ~location
  | Return ({ Return.expression; _ } as return) ->
      Return { return with Return.expression = expression >>| Expression.convert }
      |> Node.create ~location
  ...
  | With { With.items; body; async } ->
      let transform_item (item, alias) = Expression.convert item, alias >>| Expression.convert in
      With
        { With.items = List.map ~f:transform_item items; body = List.map ~f:convert body; async }
      |> Node.create ~location
  | While { While.test; body; orelse } ->
      ...
      |> Node.create ~location
  | Yield expression -> Yield (Expression.convert expression) |> Node.create ~location
  | YieldFrom expression -> YieldFrom (Expression.convert expression) |> Node.create ~location
  | Break
  | Continue
  | Global _
  | Import _
