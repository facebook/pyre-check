module Std : sig
  ...
  module SharedMem : sig
    ...
    module NoCache (UserKeyType : UserKeyType) (Value : Value.Type) :
      NoCache
      with type t = Value.t
    ...
    module WithCache (UserKeyType : UserKeyType) (Value : Value.Type) :
      WithCache
      with type t = Value.t
       and type key = UserKeyType.t
       and module KeySet = Set.Make(UserKeyType)
  ...
  module Worker : sig
    ...
    (** Raise this exception when sending work to a worker that is already busy. * We should never
        be doing that, and this is an assertion error. *)
  ...
  module MultiWorker : sig
    ...
    val next : ?max_size:int -> Worker.t list option -> 'a list -> 'a list Bucket.next
  ...
  module Daemon : sig
    ...
    (* In the absence of 'fork' on Windows, its usage must be restricted to Unix specifics parts.
       ...
       All required data must be passed through the typed channels. associated to the spawned
       process. *)
    ...
    (* Alternate entry points must be registered at toplevel, i.e. every call to
       `Daemon.register_entry_point` must have been evaluated when `Daemon.check_entry_point` is
       called at the beginning of `ServerMain.start`. *)
    ...
    (* Main function, that execute a alternate entry point. It should be called only once. Just
       before the main entry point. This function does not return when a custom entry point is
       selected. *)
