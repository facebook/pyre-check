...
(*****************************************************************************)
external hh_shared_init : config:config -> shm_dir:string option -> handle = "hh_shared_init"

let anonymous_init config = hh_shared_init ~config ~shm_dir:None
...
let rec shm_dir_init config = function
  ...
  | shm_dir :: shm_dirs -> (
      ...
      try
        ...
        hh_shared_init ~config ~shm_dir:(Some shm_dir)
      with
...
let init config =
  try anonymous_init config with
...
module Raw (Key : Key) (Value : Value.Type) : sig
  ...
  (** * Represents a set of local changes to the view of the shared memory heap * WITHOUT
      materializing to the changes in the actual heap. This allows us to * make speculative changes
      to the view of the world that can be reverted * quickly and correctly. * * A LocalChanges
      maintains the same invariants as the shared heap. Except * add are allowed to overwrite
      filled keys. This is for convenience so we * do not need to remove filled keys upfront. * *
      LocalChanges can be committed. This will apply the changes to the previous * stack, or
      directly to shared memory if there are no other active stacks. * Since changes are kept local
      to the process, this is NOT compatible with * the parallelism provided by MultiWorker.ml *)
  module LocalChanges = struct
    ...
    let rec mem stack_opt key =
      ...
      | None -> hh_mem key
      | Some stack -> (
        try Hashtbl.find stack.current key <> Remove with
        | Not_found -> mem stack.prev key )
      ...
      | Some stack -> (
        try
          ...
          | Add value ->
              value
        with
        | Not_found -> get stack.prev key )


    (** * For remove/add it is best to think of them in terms of a state machine. * A key can be in
        the following states: * * Remove: * Local changeset removes a key from the previous stack *
        Replace: * Local changeset replaces value of a key in previous stack * Add: * Local
        changeset associates a value with a key. The key is not * present in the previous stacks *
        Empty: * No local changes and key is not present in previous stack * Filled: * No local
        changes and key has an associated value in previous stack * *Error*: * This means an
        exception will occur * * * Transitions table: * Remove -> *Error* * Replace -> Remove * Add
        -> Empty * Empty -> *Error* * Filled -> Remove *)
    ...
    (** * Transitions table: * Remove -> Replace * Replace -> Replace * Add -> Add * Empty -> Add *
        Filled -> Replace *)
    ...
    let commit key =
      ...
      | None -> ()
      | Some changeset -> (
        try commit_action changeset.prev key @@ Hashtbl.find changeset.current key with
        | Not_found -> () )
...
module New : functor (Key : Key) (Value : Value.Type) -> sig
...
(Config : ConfigType) : CacheType with type key := Key.t and type value := Config.value = struct
  ...
  let collect () =
    if !size < 2 * Config.capacity then
      ...
      let l = ref [] in
      Hashtbl.iter (fun key (freq, v) -> l := (key, !freq, v) :: !l) cache;
      Hashtbl.clear cache;
...
(Config : ConfigType) : CacheType with type key := Key.t and type value := Config.value = struct
  ...
  let add x y =
    ( if !size >= Config.capacity then (* Remove oldest element - if it's still around. *)
...
module WithCache (UserKeyType : UserKeyType) (Value : Value.Type) = struct
  ...
  let get_batch keys = KeySet.fold (fun key acc -> KeyMap.add key (get key) acc) keys KeyMap.empty
  ...
  let () = invalidate_callback_list := (fun () -> Cache.clear ()) :: !invalidate_callback_list
