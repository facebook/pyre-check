...
let single_threaded_call job merge neutral next =
  ...
  while !x <> Done do
    match !x with
    | Wait ->
        (* this state should never be reached in single threaded mode, since there is no hope for
           ever getting out of this state *)
        failwith "stuck!"
    | Job l ->
...
let multi_threaded_call
  ...
  let rec dispatch workers handles acc =
    ...
    | worker :: workers -> (
      ...
      | Job bucket ->
          ...
          let handle = Worker.call worker (fun xl -> job neutral xl) bucket in
  ...
  and collect workers handles acc =
    ...
    (* Collect the results. *)
    let acc = List.fold_left ~f:(fun acc h -> merge (Worker.get_result h) acc) ~init:acc readys in
    (* And continue.. *)
    dispatch workers waiters acc
  in
