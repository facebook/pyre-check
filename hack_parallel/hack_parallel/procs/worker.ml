...
type t = {
  id: int;
  (* Simple id for the worker. This is not the worker pid: on Windows, we spawn a new worker for
     each job. *)
...
and 'a slave = {
  ...
  (* The file descriptor we might pass to select in order to wait for the slave to finish its job. *)
...
let slave_main ic oc =
  ...
  let send_result data =
    ...
    if len > 10 * 1024 * 1024 (* 10 MB *) then (
      Hh_logger.log
        "WARNING: you are sending quite a lot of data (%d bytes), which may have an adverse \
         performance impact. If you are sending closures, double-check to ensure that they have \
         not captured large values in their environment."
        len;
...
let unix_worker_main restore state (ic, oc) =
  ...
  try
    while true do
      (* Wait for an incoming job : is there something to read? But we don't read it yet. It will
         be read by the forked slave. *)
      let readyl, _, _ = Unix.select [in_fd] [] [] (-1.0) in
      if readyl = [] then exit 0;
      (* We fork a slave for every incoming request. And let it die after one request. This is the
         quickest GC. *)
...
(** Make a few workers. When workload is given to a worker (via "call" below), * the workload is
    wrapped in the calL_wrapper. *)
  ...
  let spawn _log_fd =
    ...
    let handle =
      Daemon.spawn
        ...
        entry
        (saved_state, gc_control, heap_handle)
    in
...
let call w (type a b) (f : a -> b) (x : a) : b handle =
  ...
  let result () : b =
    ...
    | _, Unix.WSTOPPED i -> Printf.ksprintf failwith "Subprocess(%d): stopped %d" slave_pid i
    | _, Unix.WSIGNALED i -> Printf.ksprintf failwith "Subprocess(%d): signaled %d" slave_pid i
  in
  ...
  let () =
    try Daemon.to_channel outc ~flush:true ~flags:[Marshal.Closures] request with
    | e -> (
      match Unix.waitpid [Unix.WNOHANG] slave_pid with
...
let is_oom_failure msg =
  String_utils.string_starts_with msg "Subprocess" && String_utils.is_substring "signaled -7" msg
...
let get_result d =
  ...
  | Processing s -> (
    try
      ...
      res
    with
    | Failure msg when is_oom_failure msg -> raise Worker_oomed
    | exn ->
        s.worker.busy <- false;
        d := Failed exn;
        raise exn )
...
let select ds =
  ...
  List.fold_right
    ~f:(fun d { readys; waiters } ->
      ...
      | Processing s when List.mem ready_fds s.infd -> { readys = d :: readys; waiters }
      | Processing _ -> { readys; waiters = d :: waiters })
    ~init:{ readys = []; waiters = [] }
    ds
