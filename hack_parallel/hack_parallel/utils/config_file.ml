...
let parse_contents contents =
  ...
  List.fold_left lines ~f:begin fun acc line ->
    ...
    then acc
      ...
      match parts with
      | [k; v] -> SMap.set (String.trim k) (String.trim v) acc
...
module Getters = struct
  ...
  let string_ key ~default config =
    Hack_option.value (SMap.get key config) ~default

  let int_ key ~default config =
    Hack_option.value_map (SMap.get key config) ~default ~f:int_of_string

  let bool_ key ~default config =
    Hack_option.value_map (SMap.get key config) ~default ~f:bool_of_string

  let string_list ~delim key ~default config =
    Hack_option.value_map (SMap.get key config) ~default ~f:(Str.split delim)
