...
(* On Win32, unwrap the handle from the 'abstract block' representing the file descriptor otherwise
   it can't be marshalled or passed as an integer command-line argument. *)
...
let init =
  (* Windows: register the serialize/desarialize functions for the custom block of
     "Unix.file_descr". *)
...
let get_handle = if Sys.win32 then raw_get_handle else Obj.magic

let wrap_handle = if Sys.win32 then raw_wrap_handle else Obj.magic
