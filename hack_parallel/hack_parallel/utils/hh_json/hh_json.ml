...
(* <value> ::= | <object> | <array> | <string> | <number> | 'true' | 'false' | 'null'
   ...
   <object> ::= '{' <members>* '}' <members> ::= <pair> { ',' <pair> }* [','] <pair> ::= <string>
   ':' <value>

   <array> ::= '{' <elements>* '}' <elements> ::= <value> { ',' <value> }* [',']

   <number> ::=

   Caveats: (+) No handling of Unicode yet (+) Numbers are just stored as strings *)
...
(* Ignore whitespace in peek/eat/next/has_more to make code that uses them cleaner *)
...
let escape s =
  ...
  |> String.iter (fun c ->
         ...
         | '\t', _ -> Buffer.add_string b "\\t"
         | _, _ when code <= 0x1f -> Printf.sprintf "\\u%04x" code |> Buffer.add_string b
...
let rec add_json_to_buffer (buf : Buffer.t) (json : json) : unit =
  match json with
  | JSON_Object l -> buf_concat ~buf ~lb:"{" ~rb:"}" ~sep:"," ~concat_elt:add_assoc_to_buffer l
  | JSON_Array l -> buf_concat ~buf ~lb:"[" ~rb:"]" ~sep:"," ~concat_elt:add_json_to_buffer l
      ...
      | JSON_Object l ->
          let nl =
            List.map l ~f:(fun (k, v) ->
                indent ^ "  " ^ json_to_string (JSON_String k) ^ ":" ^ loop (indent ^ "  ") v)
          in
          "{\n" ^ String.concat ",\n" nl ^ "\n" ^ indent ^ "}"
      | _ -> single
  in
...
let rec json_to_output oc (json : json) : unit =
  ...
  | JSON_Array l ->
      ...
      output_string oc "]"
  | JSON_String s -> output_string oc (escape s)
  | JSON_Number n -> output_string oc n
  | JSON_Bool b -> output_string oc (if b then "true" else "false")
...
module Access = struct
  ...
  let counit_with f m =
    match m with
    | Hack_result.Ok (v, _) -> v
    | Hack_result.Error e -> f e
  ...
  let catch_type_error exp f (v, keytrace) =
    ...
    | Assert_failure _ -> Hack_result.Error (Wrong_type_error (keytrace, exp))
  ...
  let get_val k (v, keytrace) =
    try
      ...
      | Some obj -> Hack_result.Ok (obj, k :: keytrace)
    with
    | Assert_failure _ -> Hack_result.Error (Not_an_object keytrace)
