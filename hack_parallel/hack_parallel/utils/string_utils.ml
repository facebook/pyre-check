...
(* Returns the index of the first occurrence of string `needle` in string `haystack`. If not found,
   returns -1.
...
let replace_char needle replacement = String.map (fun c -> if c = needle then replacement else c)
