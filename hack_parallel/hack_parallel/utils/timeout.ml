...
module Alarm_timeout = struct
  ...
  let ignore_timeout f ?timeout (ic, _pid) =
    ignore timeout;
    f ic
  ...
  let input_value_with_workaround ic =
    ...
    try Pervasives.input_value ic with
    | Failure msg as e ->
...
module Select_timeout = struct
  ...
  let create timeout = { timeout = Unix.gettimeofday () +. timeout }
  ...
  let check_timeout t = if Unix.gettimeofday () > t.timeout then raise Timeout
  ...
  let unsafe_input ?timeout tic s ofs len =
    ...
    if n <= avail then (
      ...
      avail )
    else (* No input to read, refill buffer. *)
  ...
  let input_line ?timeout tic =
    ...
    let rec scan accu len =
      ...
      if n = 0 then
        match accu with
        | [] -> raise End_of_file
        | _ -> build_result (Bytes.create len) len accu (* New line found in the buffer. *)
      else if n > 0 then (
