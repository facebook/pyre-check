...
(* The kind of a particular analysis result. Extensible by registering new
   analyses. *)
type 'a kind = ..
...
let get_kind_name kind =
  Obj.extension_constructor kind |> Obj.extension_name


let assert_constant_constructor (type a) (kind : a kind) =
  if (Obj.tag (Obj.repr kind) <> Obj.object_tag) then
    get_kind_name kind
    |> Format.sprintf "Non-constant result kind not allowed: %s"
    |> failwith


let cast (type a) (kind : a kind) =
  try
    Obj.extension_constructor kind |> Obj.extension_id
  with exn ->
    raise exn
...
let register_option abstract_kind name =
  analysis_map := (name, abstract_kind) :: !analysis_map
...
let assert_unique_kind (type a) (kind : a kind) abstract_kind =
  if Map.mem abstract_kind !kinds then
    get_kind_name kind
    |> Format.sprintf "Duplicate registration of analysis %s"
    |> failwith
