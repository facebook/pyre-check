...
(* Encapsulates the shared state maintained and computed during the outer
   fixpoint computation.

   Separates errors from function call models in order to reduce
   deserialization cost when models are looked up. Also caches the fixpoint
   state of a call model separately so it can be looked up cheaply. *)
