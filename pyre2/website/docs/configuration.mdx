...
- `search_path`: a file path describing a root from which imports should be found and imported from (including modules in `project_includes`). This this takes second highest precedence in import resolution, after `project_includes`, but before `typeshed` and `site_package_path`.
