 ...
 *
 * Modified from https://raw.githubusercontent.com/flow-typed/flow-typed/master/definitions/npm/jest_v24.x.x/flow_v0.104.x-/jest_v24.x.x.js
 * List of modifications:
 *  - added function fail
 *  - fix some [] -> Array lint warnings
 *  - make it.each/describe.each take $ReadOnlyArray instead of Array<mixed>
 *  - added `type FacebookMatchersType` to support global Fb matchers
 *  - added jest-e2e matcher types
 *  - added `type WhatsAppWebMatchersType` to support global WA Web matchers
 *  - added `type FakeTimersConfig` & `type FakeableAPI` for useFakeTimers args
 *
 * TODO(T35016336) remove the .each modifications if flow-typed adopts them
 *
 * @flow strict-local
 * @format
 * This source code is copied over from www/flow/shared/libdefs/jest.js
 */
...
/**
 * Facebook specific matchers. See setup-matchers.js for a list of
 * matchers.
 *
 * NOTE: This type and its related changes are a deviation of the original
 * `flow-typed` version of this file.
 */
type FacebookMatchersType = {
  toMatchReactSnapshot: (name?: string) => void,
  ...
};
...
type JestE2EMetricsQuery = {
  source:
    ...
    | 'tao'
    | 'delivery_db'
    | 'ig_media_insights_api'
    | 'ig_copyright_insights_api'
    | 'ig_generic_insights_api'
    | 'ig_user_ad_insights_api'
    | 'ig_user_insights_api',
  queryParams: {[key: string]: mixed},
};

type JestE2ELogSource =
  | 'sbl'
  | 'snapl'
  | 'snivy'
  | 'search_logger'
...
type JestE2EMatchersType = {
  toBePresent: () => Promise<void>,
  toBePresentCount: (count: number) => Promise<void>,
  toBePresentCountGreaterThan: (count: number) => Promise<void>,
  toBePresentCountGreaterThanOrEqual: (count: number) => Promise<void>,
  toBePresentCountLessThan: (count: number) => Promise<void>,
  toBePresentCountLessThanOrEqual: (count: number) => Promise<void>,
  toBeVisible: () => Promise<void>,
  toHaveText: (text: string) => Promise<void>,
  toContainText: (text: string) => Promise<void>,
  toHaveAttribute: (name: string, expectedValue?: any) => Promise<void>,
  toHaveProperty: (
    name: string,
    expectedValue?: string | number | boolean,
  ) => Promise<void>,
  toMatchScreenshot: (
    everstoreHandle?: any, // May be options, when set initially.
    options?: {
      exclusions?: $ReadOnlyArray<
        string | {top?: number, bottom?: number, left?: number, right?: number},
      >,
      threshold?: 'loose' | 'strict' | 'pixel-perfect',
      forceScreenshotStitchingForMobile?: boolean,
    },
  ) => Promise<void>,
  toMatchScreenshotFromFile: (
  ...
  toMatchVisually: (
    ...
    options?: {threshold?: 'loose' | 'strict' | 'pixel-perfect'},
  ) => Promise<void>,
  toHaveBeenLoggedBy: (
    source: JestE2ELogSource,
    args?: Array<any>,
    options?: {userIdOverrideForServerLogs?: string},
  ) => Promise<void>,
  toEvaluateTo: (value: any, args?: Array<any>) => Promise<void>,
  toEventuallyEqual: (fn: Function) => Promise<void>,
  toBeFocused: () => Promise<void>,
  toMatchAccessibilitySnapshot: (inlineSnapshot?: string) => Promise<void>,
  toBeMetricsResultOf: (
    metricsQuery: JestE2EMetricsQuery,
    options?: {userIdOverrideForMetrics?: string},
  ) => Promise<void>,
...
type JestExpectType = {
  ...
  toMatchInlineSnapshot: ((snapshot?: string) => void) &
    ((propertyMatchers?: any, snapshot?: string) => void),
  /**
...
type JestObjectType = {
  ...
  requireActual: (<T>(
    m: $Flow$ModuleRef<T>,
    // $FlowIgnore
  ) => $Flow$ModuleRef$UnwrapWWWSpecific<T>) &
    (<T>(m: string) => T),
  /**
