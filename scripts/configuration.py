...
class Configuration:
    ...
    def _apply_defaults(self) -> None:
        ...
        if not self._binary:
            LOG.info('No binary specified, looking for `pyre.bin` in PATH')
            self._binary = shutil.which("pyre.bin")
            if not self._binary:
                LOG.warning("Could not find `pyre.bin` in PATH")
            else:
        ...
        if not self._typeshed:
            ...
            self._typeshed = self._find_typeshed()
            if not self._typeshed:
    ...
    def _find_typeshed(self) -> Optional[str]:
        ...
        while True:
            ...
            typeshed = os.path.join(directory, "typeshed/")
            LOG.debug("Trying with: `%s`", typeshed)
            if os.path.isdir(typeshed):
                return typeshed
        return None
