...
let run_infer ~scheduler ~configuration ~global_resolution qualifiers =
  ...
  let should_infer ~configuration:{ Configuration.Analysis.ignore_infer; _ } relative =
    try
      let path = Path.create_absolute relative in
  ...
  let map _ qualifiers =
    let analyze_source
        (errors, number_files)
        ({ Ast.Source.source_path = { Ast.SourcePath.relative; _ }; _ } as source)
      =
      if should_infer ~configuration relative then
        errors, number_files
      else
