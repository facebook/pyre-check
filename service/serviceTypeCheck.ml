...
let analyze_source
  ...
  if version < 3 || autogenerated then
    begin
      ...
      { TypeCheck.errors = []; lookup = None; type_coverage = Coverage.create () }
    end
  else
    begin
  ...
  |> Scheduler.map_reduce
            ...
            fun { errors; lookups; number_files; type_coverage = total_type_coverage } handle ->
              ...
              | Some source ->
                  ...
                  in
                  {
                    ...
                    type_coverage = Coverage.sum total_type_coverage type_coverage;
                  }
              | None -> {
                  errors;
                  lookups;
    ...
    ~reduce:(fun left right ->
        ...
        Log.info "Processed %d of %d sources" number_files (List.length handles);
        {
          ...
          number_files;
          type_coverage = Coverage.sum left.type_coverage right.type_coverage;
        })
  ...
  | false ->
      ...
      let analyze_and_postprocess
        ...
        (errors :: current_errors,
         ...
         (Coverage.sum total_type_coverage type_coverage))
      in
      List.fold
