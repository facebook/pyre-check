...
let analyze_source
  ...
  if version < 3 || autogenerated then
    begin
      ...
      { TypeCheck.Result.errors = []; lookup = None; coverage = Coverage.create () }
    end
  else
    begin
...
let analyze_sources_parallel
  ...
  |> Scheduler.map_reduce
    ...
    ~map:(fun _ handles ->
        let result =
          List.fold ~init ~f:(
            fun { errors; lookups; number_files; coverage = total_coverage } handle ->
              match AstSharedMemory.get_source handle with
              | Some source ->
                  let { TypeCheck.Result.errors = new_errors; lookup; coverage; } =
