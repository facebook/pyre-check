  ...
  let analyze_source
    ...
    if version < 3 || autogenerated then
      ...
      begin
        ...
        Log.log ~section:`Check "Checking `%s`..." handle;
        let result =
        ...
        Statistics.performance
          ...
          ~section:`Check
          ~name:(Format.asprintf "SingleFileTypeCheck of %s" handle)
          ~timer
          ~normals:["handle", handle; "request kind", "SingleFileTypeCheck"]
  ...
  let handles =
    ...
    let filter_by_root handle =
      ...
      | Some { Source.handle; _ } ->
          ...
          let relative = Path.create_relative ~root:local_root ~relative:handle in
