    ...
    [
      ...
      @@ assert_type_errors
           ...
           {|
            ...
            |}
           [
             "Incompatible return type [7]: Expected `int` but got `unknown`. Type `int` expected \
              on "
      ...
      @@ assert_type_errors
           ...
           [
             ...
             "Missing attribute annotation [4]: Attribute `bar` of class `Foo` has no type \
              specified.";
      ...
      @@ assert_type_errors
           ...
           [
             "Incompatible attribute type [8]: Attribute `attribute` declared in class `Foo` has \
              type `int` but is used as type `str`.";
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Missing attribute annotation [4]: Attribute `n` of class `Foo` has type `None` but \
              no type is specified.";
             "Missing attribute annotation [4]: Attribute `l` of class `Foo` has type \
              `typing.Type[list]` but no type is specified.";
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible attribute type [8]: Attribute `__match_args__` declared in class `Foo` \
              has type `Tuple[typing_extensions.Literal['x'], typing_extensions.Literal['y']]` but \
              is used as type `Tuple[typing_extensions.Literal['x']]`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
    ...
    [
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Invalid decoration [56]: Invalid property setter `y.setter`: `y` does not match \
              decorated method `x`.";
           ];
    ]


let test_attribute_strict =
  test_list
    [
...
let test_check_attribute_initialization =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Uninitialized attribute [13]: Attribute `x` inherited from abstract class `Foo` in \
              class `Bar` to have type `int` but is never initialized.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
           ...
           [
             "Uninitialized attribute [13]: Attribute `x` inherited from abstract class `Foo` in \
              class `Bar` to have type `int` but is never initialized.";
             "Uninitialized attribute [13]: Attribute `y` inherited from abstract class `Foo` in \
              class `Bar` to have type `int` but is never initialized.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
      ...
      @@ assert_type_errors
           ...
           [
             "Uninitialized attribute [13]: Attribute `x` inherited from abstract class `Foo` in \
              class `Baz` to have type `int` but is never initialized.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [];
      (* TODO(T98000466): `__init_subclass__` should not instantiate attributes for the current
         class when it is non abstract, only subclasses. Add an error here. *)
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
              from typing import ClassVar
...
let test_check_missing_attribute =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Missing attribute annotation [4]: Attribute `a` of class `Foo` has type `int` but no \
              type is specified.";
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incomplete type [37]: Type `typing.List[Variable[_T]]` inferred for `self.a` is \
...
let test_attribute_type_variable_resolution =
  test_list
    [
      ...
      @@ assert_type_errors
          ...
          |}
           [
             "Uninitialized attribute [13]: Attribute `value` is declared in class `Wrapper` to \
              have "
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible attribute type [8]: Attribute `x` declared in class `A` has type `str` \
              but is used as type `int`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
...
let test_getattr_literal_access =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
...
let test_check_metaclass_attributes =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
    ...
    [
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Inconsistent override [15]: `a` overrides attribute defined in `C` inconsistently. \
...
let test_class_with_same_name_as_local_variable =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
...
let () =
  ...
  >::: [
         ...
         test_attribute_decorators;
         test_attribute_strict;
         test_check_attribute_initialization;
         test_check_missing_attribute;
         test_attribute_type_variable_resolution;
         test_check_getattr;
         test_getattr_literal_access;
         test_check_metaclass_attributes;
         test_check_annotated;
         test_class_with_same_name_as_local_variable;
       ]
  |> Test.run
