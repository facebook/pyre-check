    ...
    [
      ...
      @@ assert_type_errors
           {|
              ...
              |}
           [
             "Uninitialized attribute [13]: Attribute `x` is declared in class `A` to have type \
              `int` but is never initialized.";
             "Uninitialized attribute [13]: Attribute `y` is declared in class `A` to have type \
              `int` but is never initialized.";
             "Invalid assignment [41]: Cannot reassign final attribute `self.x`.";
      ...
      @@ assert_type_errors
           {|
              ...
              |}
           [
             "Incompatible attribute type [8]: Attribute `x` declared in class `A` has type `int` \
...
let () = "final" >::: [test_final_methods; test_final_class; test_final_attributes] |> Test.run
