...
let test_check_attributes_from_unknown_imports =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
...
let test_check_inverse_operator =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Unsupported operand [58]: `<` is not supported for operand types `int` and \
              `Optional[int]`.";
             "Unsupported operand [58]: `<` is not supported for operand types `Optional[int]` and \
              `int`.";
             "Unsupported operand [58]: `+` is not supported for operand types `int` and \
              `Optional[int]`.";
             "Unsupported operand [58]: `+` is not supported for operand types `Optional[int]` and \
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible parameter type [6]: In call `int.__lt__`, for 1st positional argument, \
      ...
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           (* Any % int should return unknown, not int (via the int __mod__ operator). This is
              necessary for dealing with edge cases with old-style format strings when the format
              string happens to be typed as Any. *)
           {|
              import typing
      ...
      (* When an object has type `Any`, we should not use the inverse operator since the `Any`
         object might have the original operator. Just treat it as an unknown callable. *)
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
...
let test_check_method_parameters =
  test_list
    [
      (* Calls to methods *)
      labeled_test_case __FUNCTION__ __LINE__
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible parameter type [6]: In call `str.substr`, for 1st positional argument, \
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible parameter type [6]: In call `bar`, for 1st positional argument, \
              expected `int` but got `str`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
      ...
      @@ assert_strict_type_errors
           ...
           [
             ...
             "Revealed type [-1]: Revealed type for `test.StringEnumTwo[\"key\"]` is \
              `StringEnumTwo`.";
           ];
      (* Defining methods *)
      labeled_test_case __FUNCTION__ __LINE__
      ...
      @@ assert_type_errors
           ...
           [
             "Revealed type [-1]: Revealed type for `test.Foo.bar` is \
              `List[typing.ClassMethod[typing.Callable(Foo.bar)[[Named(cls, typing.Type[Foo])], \
              None]]]`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           ...
           [
             "Revealed type [-1]: Revealed type for `test.Foo().__init_subclass__` is \
              `BoundMethod[typing.Callable(Foo.__init_subclass__)[[Named(cls, typing.Type[Foo])], \
              None], typing.Type[Foo]]`.";
             "Revealed type [-1]: Revealed type for `test.Foo().__class_getitem__` is \
              `BoundMethod[typing.Callable(Foo.__class_getitem__)[[Named(cls, typing.Type[Foo])], \
              None], typing.Type[Foo]]`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           ...
           [
             "Revealed type [-1]: Revealed type for `test.Foo().__init_subclass__` is \
              `List[typing.Callable(Foo.__init_subclass__)[[Named(cls, typing.Type[Foo])], \
              None]]`.";
             "Revealed type [-1]: Revealed type for `test.Foo().__class_getitem__` is \
      ...
      @@ assert_type_errors
           ...
           {|
            ...
            |}
           [
             "4: Incompatible parameter type [6]: In call `foo`, for 3rd positional argument, \
              expected `str` but got `int`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible parameter type [6]: In call `never_input`, for 1st positional argument, \
...
let test_check_abstract_methods =
  let other_sources =
    [
      {
        handle = "abc.pyi";
...
let test_check_behavioral_subtyping__strengthened_postcondition =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
      ...
      (* Method resolution order. TODO(T124880784): the reason Pyre behaves this way in terms of
         ...
         resolution order. But this is not actually sound - an instance of Bar is a valid value of
         type Foo2, and therefore the `foo_legal_override` function is in fact not substitutable. *)
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
             class Foo1:
             ...
             |}
           [
             "Inconsistent override [15]: `test.Bar.foo_illegal_override` overrides method defined \
              in `Foo1` inconsistently. Returned type `str` is not a subtype of the overridden \
              return `int`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
...
let test_check_behavioral_subtyping__weakened_precondition =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
...
let test_check_behavioral_subtyping__overloads =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      ...
      @@ assert_default_type_errors
           ...
           [
             "Inconsistent override [14]: `test.Bar.foo` overrides method defined in `Foo` \
              inconsistently. Parameter of type `typing.Union[A, B]` is not a supertype of the \
              overridden parameter `typing.Union[A, B, C]`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_default_type_errors
           {|
...
let test_check_behavioral_subtyping__special_cases =
  test_list
    [
      ...
      @@ assert_type_errors
           ...
           [
             "Inconsistent override [14]: `test.Bar.__dunder__` overrides method defined in `Foo` \
              inconsistently. Parameter of type `int` is not a supertype of the overridden \
              parameter `float`.";
           ];
      ...
      @@ assert_default_type_errors
           {|
             ...
             |}
           [
             "Inconsistent override [14]: `test.Circle.with_scale` overrides method defined in \
              `Shape` inconsistently. Parameter of type `int` is not a supertype of the overridden \
              parameter `float`.";
           ];
    ]


let test_check_behavioral_subtyping__attribute_kinds =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
...
let test_check_nested_class_inheritance =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
...
let test_check_method_resolution =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
      ...
      @@ assert_type_errors
           ...
           [
             "Revealed type [-1]: Revealed type for `x.__iadd__` is \
              `BoundMethod[typing.Callable(Foo.__add__)[[Named(self, Foo[int]), Named(other, \
              Foo[int])], Foo[int]], Foo[int]]`.";
             "Unsupported operand [58]: `+` is not supported for operand types `Foo[int]` and \
              `Foo[str]`.";
           ];
    ]
    ...
    [
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible variable type [9]: x is declared to have type `typing.Callable(foo)[[], \
              None]` but is used as type `typing.Callable(bar)[[], None]`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
...
let test_check_callable_protocols =
  test_list
    [
      ...
      @@ assert_type_errors
           ...
           [
             "Incompatible return type [7]: Expected `None` but got "
             ^ "`BoundMethod[typing.Callable(Foo.bar)[[Named(self, Type[Foo]), Named(x, int)], \
                str], Type[Foo]]`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible parameter type [6]: In call `Call.__call__`, for 1st positional \
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [];
      (* Calling is a special method access of `__call__`, thus calling Type[X] prefers a `__call__`
         in a metaclass over both the constructor and a `__call__` on X *)
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
              from typing import Type
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible variable type [9]: f is declared to have type `typing.Callable[[int], \
              C]` but is used as type `Type[C]`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_default_type_errors
           {|
...
let test_check_explicit_method_call =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible parameter type [6]: In call `Class.method`, for 1st positional \
              argument, expected `Class` but got `object`.";
           ];
    ]
...
let test_check_override_decorator =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Invalid override [40]: `test.Bar.foo` is decorated with @override, but no method of \
              the same name exists in superclasses of `Bar`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
          ...
          |}
           [
             "Invalid override [40]: Non-static method `test.Bar.same_method` cannot override a \
              static method defined in `Foo`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Invalid override [40]: `test.foo` is illegally decorated with @override: @override \
              may only be applied to methods, but this element is not a method.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
             ...
             |}
           [
             "Invalid override [40]: `foo_inner` is illegally decorated with @override: @override \
              may only be applied to methods, but this element is not a method.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Invalid override [40]: `test.Bar.method0` is decorated with @override, but no method \
              of the same name exists in superclasses of `Bar`.";
             "Invalid override [40]: `test.Bar.method1` is decorated with @override, but no method \
              of the same name exists in superclasses of `Bar`.";
           ];
    ]


let test_check_static =
  test_list
    [
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible parameter type [6]: In call `Foo.foo`, for 1st positional argument, \
              expected `int` but got `str`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible parameter type [6]: In call `Foo.classmethod`, for 1st positional \
              argument, expected `int` but got `str`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible parameter type [6]: In call `Foo.staticmethod`, for 1st positional \
              argument, expected `int` but got `str`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible parameter type [6]: In call `Foo.instancemethod`, for 2nd positional \
              argument, expected `int` but got `str`.";
           ];
    ...
    [
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Revealed type [-1]: Revealed type for `1 in test.WeirdGetItem()` is \
              `typing.List[int]`.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
    ...
    [
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Incompatible parameter type [6]: In call `expect_string`, for 1st positional \
              argument, expected `str` but got `int`.";
           ];
    ]


let test_check_private_member_access =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
      ...
      @@ assert_type_errors
           ...
           [
             ...
             "Undefined attribute [16]: `Child` has no attribute `__private`. `__private` looks \
      ...
      @@ assert_type_errors
           ...
           [
             "Undefined attribute [16]: `Child` has no attribute `__private_method`. \
              `__private_method` looks like a private attribute, which is not accessible from \
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Undefined attribute [16]: `Foo` has no attribute `__private`. `__private` looks like \
              a private attribute, which is not accessible from outside its parent class.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
           {|
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Undefined attribute [16]: Item `Base` of `typing.Union[Base, Child]` has no \
              attribute `__private`. `__private` looks like a private attribute, which is not \
              accessible from outside its parent class.";
             "Undefined attribute [16]: `Base` has no attribute `__private`. `__private` looks \
              like a private attribute, which is not accessible from outside its parent class.";
             "Undefined attribute [16]: `Child` has no attribute `__private`. `__private` looks \
              like a private attribute, which is not accessible from outside its parent class.";
           ];
      ...
      @@ assert_type_errors
           ...
           [
             ...
             "Undefined attribute [16]: `GenericChild` has no attribute `__private`. `__private` \
              looks like a private attribute, which is not accessible from outside its parent \
              class.";
             "Incompatible return type [7]: Expected `Variable[T]` but got `unknown`.";
             "Undefined attribute [16]: `GenericBase` has no attribute `__private`. `__private` \
              looks like a private attribute, which is not accessible from outside its parent \
              class.";
           ];
      labeled_test_case __FUNCTION__ __LINE__
      ...
      @@ assert_type_errors
           ...
           [
             "Undefined attribute [16]: `ExampleClass` has no attribute \
              `_Child__private_static_method`.";
             "Undefined attribute [16]: `Child` has no attribute `__private_static_method`. \
              `__private_static_method` looks like a private attribute, which is not accessible \
              from outside its parent class.";
             "Undefined attribute [16]: `Child` has no attribute `__private_static_method`. \
              `__private_static_method` looks like a private attribute, which is not accessible \
              from outside its parent class.";
             "Undefined attribute [16]: `Child` has no attribute `__private_static_method`. \
...
let test_enforce_dunder_params =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
    ...
    [
      ...
      @@ assert_type_errors
           {|
            ...
            |}
           [
             "Analysis failure [30]: Pyre gave up inferring types for some variables because \
              function `test.bar` was too complex.";
...
let test_nested_function_shows_function_name_in_errors =
  test_list
    [
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_type_errors
...
let () =
  "method"
  >::: [
         test_check_method_returns;
         test_check_attributes_from_unknown_imports;
         test_check_inverse_operator;
         test_check_method_parameters;
         test_check_private_member_access;
         test_check_abstract_methods;
         test_check_behavioral_subtyping__strengthened_postcondition;
         test_check_behavioral_subtyping__weakened_precondition;
         test_check_behavioral_subtyping__overloads;
         test_check_behavioral_subtyping__special_cases;
         test_check_behavioral_subtyping__attribute_kinds;
         test_check_nested_class_inheritance;
         test_check_method_resolution;
         test_check_callables;
         test_check_callable_protocols;
         test_check_explicit_method_call;
         test_check_self;
         test_check_meta_self;
         test_check_setitem;
         test_check_override_decorator;
         test_check_static;
         test_check_in;
         test_check_enter;
         test_enforce_dunder_params;
         test_fixpoint_threshold;
