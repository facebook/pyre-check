    ...
    let resolution, errors =
      ...
      | BooleanOperator { BooleanOperator.left; operator; right } -> (
          let update resolution expression =
            forward_assert ~resolution expression
            |> function
          ...
          match operator with
          | BooleanOperator.And ->
              let left_state, _ = forward_assert ~resolution left in
              let right_state =
                left_state
                |> function
                | Unreachable -> Unreachable
                | Value resolution -> forward_assert ~resolution right |> fst
              in
      ...
      | ComparisonOperator
          ...
          forward_assert ~resolution left
      | ComparisonOperator
          {
      ...
      | WalrusOperator { target; _ } ->
          let resolution, _ = forward_assert ~resolution target in
          resolution, errors
      | Call
          {
            arguments = { Call.Argument.name = None; value = { Node.value = Name name; _ } } :: _;
            _;
          }
        when is_simple_name name -> (
                      ...
                      | `Attribute ({ Name.Attribute.base; attribute; _ }, resolved) ->
                          ...
                          let reference =
                            ...
                            | { Node.value = Name name; _ } when is_simple_name name ->
                                Some
                                  (Reference.create ~prefix:(name_to_reference_exn name) attribute)
                            | _ ->
                                parent_class_name
                                >>| Reference.create
                          ...
                          let target_annotation =
                            ...
                            | _ ->
                                (* The reason why we need to do resolve_expression on the entire
                                   target again is to deal with imported globals. To fix it, we
                                   ought to stop representing imported globals as
                          ...
                          let check_previously_annotated errors =
                            ...
                            | Name.Identifier identifier ->
                                ...
                                let is_reannotation_with_same_type =
                                  (* TODO(T77219514): special casing for re-annotation in loops can
                                     be removed when fixpoint is gone *)
                                           ...
                                           missing_annotation =
                                             {
                                               ...
                                               annotation = actual_annotation;
                                               given_annotation =
                                                 Option.some_if is_immutable expected;
                                               evidence_locations;
                                               thrown_at_source;
                                             };
                                         }),
