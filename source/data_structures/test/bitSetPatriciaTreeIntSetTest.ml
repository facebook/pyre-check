...
include Data_structures.BitSetPatriciaTreeIntSet.Make (struct
  let common_integers = [| 2; 4; 6; 8; 10; 12; 14 |]
end)
...
let test_add _ =
  ...
  let set = add 2 (add 0 empty) in
  assert_equal (is_empty set) false;
  assert_equal (cardinal set) 2;
  assert_equal (mem 0 set) true;
  assert_equal (mem 1 set) false;
  assert_equal (mem 2 set) true;
  assert_equal (elements set) [0; 2];

  let set = add 2 (add 2 (add 0 empty)) in
  assert_equal (is_empty set) false;
  assert_equal (cardinal set) 2;
  assert_equal (mem 0 set) true;
  assert_equal (mem 1 set) false;
  ...
  assert_equal (elements set) [1; 0; 10]
...
let test_singleton _ =
  ...
  let set = singleton 0 in
  assert_equal (is_empty set) false;
  assert_equal (cardinal set) 1;
  assert_equal (mem 0 set) true;
  assert_equal (mem 10 set) false;
  assert_equal (elements set) [0]
...
let test_remove _ =
  ...
  let set = remove 2 (add 2 empty) in
  assert_equal (is_empty set) true;
  assert_equal (cardinal set) 0;
  assert_equal (mem 2 set) false;
  assert_equal (elements set) [];

  let set = remove 3 (add 2 empty) in
  assert_equal (is_empty set) false;
  assert_equal (cardinal set) 1;
  assert_equal (mem 1 set) false;
  assert_equal (mem 2 set) true;
  assert_equal (mem 3 set) false;
  assert_equal (elements set) [2];

  let set = remove 3 (add 3 empty) in
  assert_equal (is_empty set) true;
  assert_equal (cardinal set) 0;
  assert_equal (mem 3 set) false;
  assert_equal (elements set) [];
...
let test_of_list _ =
  ...
  assert_equal (elements set) [7; 3; 5; 9; 1; 0; 8; 6; 4; 2]
