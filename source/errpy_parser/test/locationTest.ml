  ...
  test_list
    [
      ...
      (*TODO (T148669698): assert_parsed "a = 1 # type: int" ~expected: [ node ~start:(1, 0)
        ~stop:(1, 18) (Statement.Assign { Assign.target = node ~start:(1, 0) ~stop:(1, 1)
        (Expression.Name (Name.Identifier "a")); annotation = Some (node ~start:(1, 6) ~stop:(1, 18)
        (Expression.Constant (Constant.String (StringLiteral.create "int")))); value = node
        ~start:(1, 4) ~stop:(1, 5) (Expression.Constant (Constant.Integer 1)); }); ];*)
      labeled_test_case __FUNCTION__ __LINE__
      @@ assert_parsed
           "a: int"
           ~expected:
...
let test_type_comments _context =
