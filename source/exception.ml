...
(* Converts back to an `exn` with the right backtrace. Generally, avoid this in favor of the helpers
   in this module, like `to_string` and `get_backtrace_string`. *)
let to_exn t =
  try reraise t with
  | exn -> exn


(* Like `wrap`, but for the unusual case where you want to create an `Exception` for an un-raised
   `exn`, capturing its stack trace. If you've caught an exception, you should use `wrap` instead,
   since it already has a stack trace. *)
...
(** `internal_get_backtrace_slots_of_callstack x y` returns the top `y` backtrace slots after
    skipping the top `x` slots. Skipping slots is useful for hiding our own exception printing
    functions. *)
let internal_get_backtrace_slots_of_callstack ~skip n =
  (* skip `Printexc.get_callstack` *)
  let module BasePrintexc = Printexc in
  let open Core in
  let skip = skip + 1 in
  let n =
    if Int.max_value - skip < n then
      Int.max_value
    else
      n + skip
  in
  let callstack = BasePrintexc.get_callstack n in
  match BasePrintexc.backtrace_slots callstack with
  | Some callstack_slots ->
      if Array.length callstack_slots > skip then
        Some (Array.sub callstack_slots ~pos:skip ~len:(Array.length callstack_slots - skip))
      else
        None
  | None -> None


(** Appends the current callstack (up to `n` slots, after skipping `skip` frames) to the given
    backtrace. *)
