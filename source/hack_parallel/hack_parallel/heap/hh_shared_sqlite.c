...
void make_all_tables(sqlite3* db) {
  ...
  for (int i = 0; i < ARRAY_SIZE(create_tables_sql); ++i) {
    assert_sql(
        sqlite3_exec(db, create_tables_sql[i], NULL, 0, NULL), SQLITE_OK);
  }
  return;
}
...
void hhfi_insert_row(
  ...
  assert_sql(sqlite3_bind_text(stmt, 2, name, -1, SQLITE_TRANSIENT), SQLITE_OK);
  assert_sql(sqlite3_bind_int64(stmt, 3, kind), SQLITE_OK);
  assert_sql(
      sqlite3_bind_text(stmt, 4, filespec, -1, SQLITE_TRANSIENT), SQLITE_OK);
