...
let collect_main_issues ~taint_configuration ~issue_handle_map =
  ...
  let accumulate so_far ~all_issues =
    ...
    let main_issue =
      match main_issues with
      | [] -> failwith "Expected at least one main issue."
      | [main_issue] -> main_issue
      | main_issue :: _ ->
          Log.warning
            "Expect exactly one main issue. However, we get the following issues: %s"
            (List.map main_issues ~f:(fun issue -> IssueHandle.show issue.handle)
            |> String.concat ~sep:"\n");
          main_issue
    in
    List.fold secondary_issues ~init:so_far ~f:(fun so_far secondary_issue ->
