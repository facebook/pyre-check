...
let parse_untrimmed
  ...
  try
    let source =
      ...
      let metadata =
        let raw_hash = [%hash: string list] (String.split source ~on:'\n') in
        Source.Metadata.create_for_testing
          ~autogenerated
          ~debug
          ~local_mode
          ~ignore_lines
...
let parse
  ...
  |> parse_untrimmed ~handle ~debug ~local_mode ~version ~docstring ~coerce_special_methods
